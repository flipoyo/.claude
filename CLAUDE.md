...
| `memory/` | Everything a workspace remembers: the State area (`states.py`), the hash-chained ledger (`ledger_entry.py`, `ledger_store.py`), what `verify` checks (`integrity.py`), the State writer and the legacy single-file register kept for reading (`store.py`). Every command that writes a State appends one entry, carrying the toolchain that produced it. **Nothing here runs Git** — when a memory becomes a repository, that work belongs to `operations.py`/`git_runner.py`, driven by this package. `snapshot_resolver.py` stays outside it. |
