...
| `git_runner.py` | Git subprocess wrapper — the sole `import subprocess` module, and the sole owner of how Git's output is decoded (`errors="replace"` at both wrappers; `_query_bytes` for callers that must search raw bytes). `merge`/`fetch` are operations; `can_merge_cleanly`/`branch_known` are read-only questions that never touch a worktree, which is what lets a preflight ask about every repo before acting on any. |
