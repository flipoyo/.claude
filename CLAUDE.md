...
| `ledger_entry.py`, `integrity.py`, `ledger_store.py` | Hash-chained register mechanics — entry construction, chain verification, atomic per-entry persistence — and **the register ComplexGitSync writes**: every command that writes a State appends one entry to `.cgitsync/lgr/`. Each entry carries the toolchain that produced it (cgitsync, git, pixi, dvc, git-lfs), inside the entry hash. One canonicalisation, in `ledger_entry.compute_entry_hash`, used by writer and verifier alike. The single-file `.lgr` register is read for old workspaces and written by nothing. |
